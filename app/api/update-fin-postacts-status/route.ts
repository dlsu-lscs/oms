...
export async function POST(req: NextRequest) {
  try {
    ...
    // Update status
    await pool.query(
      'UPDATE event_trackers SET fin_postacts_status = ? WHERE event_id = ?',
      [status, eventId]
    );

    return new NextResponse('Finance postacts status updated successfully', { status: 200 });
  } catch (error: any) {
    console.error("Error updating finance postacts status:", error);
